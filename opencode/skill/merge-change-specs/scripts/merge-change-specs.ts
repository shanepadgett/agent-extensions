...
import { validateChangeSpecMarkdown } from "../../../plugin/spec-validate";
...
function parseArgs(argv: string[]): { change: string; dryRun: boolean } {
  ...
  if (!change) {
    die(
      "Missing required --change <name>. Example: bun opencode/skill/merge-change-specs/scripts/merge-change-specs.ts --change auth-refresh",
    );
  }
