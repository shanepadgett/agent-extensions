---
name: keep-current
description: Research current online documentation and information using Codex tools (no subagents).
---

# Keep Current (Codex)

Ensure responses are accurate and up-to-date by doing targeted research directly, without background agents.
...
## Research Workflow (Codex)

When research is needed, do it directly:

1. **Clarify the question**: restate what you need and any constraints.
2. **Find authoritative sources**: prefer official docs, maintainers, or standards.
3. **Cross-check**: verify across multiple sources when possible.
4. **Synthesize**: return a concise summary with sources if the runtime supports citations.
...
- Return distilled findings with source URLs when possible
